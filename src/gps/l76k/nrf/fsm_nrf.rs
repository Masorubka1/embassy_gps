...
impl<IO> L76kFsm<IO, NrfOutput<'static>, NrfOutput<'static>>
where
    IO: Read + Write + ErrorType,
{
    /// Creates an nRF FSM from hardware pins and UART factory.
    pub async fn new_nrf<REINIT, STANDBY, F>(hw: GpsHw<REINIT, STANDBY>, make_io: F) -> Self
    where
        REINIT: embassy_nrf::gpio::Pin,
        STANDBY: embassy_nrf::gpio::Pin,
        F: FnOnce() -> IO,
    {
...
impl<IO: Read + Write + ErrorType> GpsFsm for L76kFsm<IO, NrfOutput<'static>, NrfOutput<'static>> {
    type Event = GpsEvent;
